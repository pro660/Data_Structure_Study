...
static void add_document(const char* path, int doc_id) {
    g_docs[doc_id].path = strdup(path);
...
static void run_query(const char* qline) {
    ...
    if (acc.size == 0) {
        ...
        char ts[32]; { time_t tt = time(NULL); struct tm tmv; gmtime_s(&tmv, &tt); strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%SZ", &tmv); }
...
int main(int argc, char** argv) {
    int input_docs = (argc >= 2) ? (argc - 1) : 0;
    g_doc_count = input_docs + 1;
    g_docs = (Document*)calloc(g_doc_count, sizeof(Document));

    for (int i = 1;i < argc;i++) { add_document(argv[i], i - 1); index_file(argv[i], i - 1); }
